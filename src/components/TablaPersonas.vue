      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="persona in personas" :key="persona.id">
            ...
            <td v-if="editando === persona.id">
                <button class="btn btn-success" v-on:click="guardarPersona(persona)">💾 Guardar</button>
                <button class="btn btn-secondary ml-2" v-on:click="cancelarEdicion(persona)">❌ Cancelar</button>
            </td>
            <td v-else>
                <button class="btn btn-info" v-on:click="editarPersona(persona)">✏️ Editar</button>
                <button class="btn btn-danger ml-2" v-on:click="$emit('delete-persona', persona.id)">🗑️ Eliminar</button>
            </td>
